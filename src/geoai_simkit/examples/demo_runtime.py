...
def build_demo_runtime_plan(profile: str = "auto", *, device: str | None = None) -> DemoRuntimePlan:
    ...
    if requested == 'gpu' and has_cuda and not requested_device.startswith('cpu'):
        meta: dict[str, object] = {'compute_profile': 'auto', 'control_strategy': 'core-gated', 'execution_profile': 'gpu', 'execution_note': 'Using the richer GPU-capable staged solver profile.', 'demo_execution_profile': 'gpu', 'demo_execution_note': 'Using the richer GPU-capable staged solver profile.'}
