...
class _PVStub:  # pragma: no cover
    # Keep these names for postponed annotations without importing PyVista/VTK
    # during headless CLI, solver and test workflows. Importing PyVista eagerly
    # can be slow or can hang on systems without a working OpenGL/VTK stack.
...
def _is_multiblock_mesh(data: Any) -> bool:
    """Return True for PyVista-like MultiBlock objects without importing PyVista.

    The core model is used by headless solvers and CLI smoke tests, so it must
    not import PyVista just to check a type. This duck-typed predicate keeps the
    GUI path compatible while allowing lightweight grid stubs in regression
    cases.
    """
    if data is None:
        return False
    if type(data).__name__.lower() == 'multiblock':
        return True
    return bool(hasattr(data, 'combine') and hasattr(data, 'keys') and not hasattr(data, 'get_cell'))
...
@dataclass(slots=True)
class StageAction:
    """Object-level staged construction action.

    Region activation fields are still supported, but new GUI workflows should
    record explicit actions on blocks, supports, interfaces, loads or boundary
    conditions so that split/merge operations can migrate intent safely.
    """
...
class SimulationModel:
    ...
    def translate_object_blocks(self, object_keys: Iterable[str], offset: tuple[float, float, float]) -> int:
        ...
        """
        if not _is_multiblock_mesh(self.mesh):
            return 0
    ...
    def material_for_region(self, region_name: str) -> MaterialBinding | None:
        ...
        for item in self.materials:
            if normalize_region_name(item.region_name) == target:
                return item
        family_aliases = {
            'soil': ('soil_', 'continuum_soil'),
            'wall': ('wall', 'retaining_wall'),
        }
        for alias, prefixes in family_aliases.items():
            if target == alias or any(target.startswith(prefix) or target == prefix for prefix in prefixes):
                for item in self.materials:
                    item_name = normalize_region_name(item.region_name)
                    if item_name == alias or any(item_name.startswith(prefix) or item_name == prefix for prefix in prefixes):
                        return item
        return None
    ...
    def apply_result_to_mesh(self, result: ResultField) -> None:
        ...
        if _is_multiblock_mesh(self.mesh):
            self.mesh = grid
        target = grid.point_data if result.association == 'point' else grid.cell_data
        target[name] = result.values
        if not result.stage or str(result.stage).strip().lower() in {'default', 'initial'}:
            target[result.name] = result.values
